...
const Dashboard = () => {
  ...
  const createSpendDistributionChartData = () => {
    if (!overviewData || !overviewData.campaigns) return null;
  ...
  const createGaUsersChartData = () => {
    ...
    const avgSessionDurationIndex = metricHeaders.findIndex(h => h.name === 'averageSessionDuration');
  ...
  const createUserEngagementChartData = () => {
    ...
    const engagementRateData = gaData.engagementMetrics.map(day => 
      parseFloat(day.engagementRate || 0) * 100
    );
    
    const avgSessionDurationData = gaData.engagementMetrics.map(day => 
    ...
    const { limitedLabels: limitedLabels1, limitedData: limitedEngagementRate } = 
      limitDataPoints(engagementRateData, rawLabels);
    
    const { limitedData: limitedSessionDuration } = 
      limitDataPoints(avgSessionDurationData, rawLabels, limitedLabels1.length);
